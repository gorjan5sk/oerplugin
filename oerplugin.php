...
function oerplugin_activate() {
  ...
  $user_id = oer_create_page( __('Мои ресурси','oerplugin') , "useroer", 2010);
  $user_upload_id = oer_create_page( __('Прикачи ресурс','oerplugin') , "upload", 2011, $user_id);
  $user_resources_id = oer_create_page( __('Уредувај ресурси','oerplugin'), "myresources", 2013, $user_id);
  $user_profile_id = oer_create_page( __('Уреди профил','oerplugin'), "profile", 2012, $user_id);
  $user_logout_id = oer_create_page(__('Одјави ме','oerplugin'), "logout", 2014, $user_id);
